...
SELECT COUNT(*) AS SOLUONGHOADON FROM HOADON HD
WHERE MAKH NOT IN (
	SELECT MAKH FROM KHACHHANG KH
	WHERE KH.MAKH = HD.MAKH
);

--21. Có bao nhiêu sản phẩm khác nhau được bán ra trong năm 2006.
SELECT COUNT(DISTINCT CT.MASP) AS SOLUONGSANPHAM FROM CTHD CT
...
SELECT AVG(TRIGIA) AS TRIGIATB FROM HOADON
...
WHERE YEAR(HD.NGHD) = 2006 AND HD.TRIGIA = (SELECT MAX(TRIGIA) FROM HOADON)
...
SELECT MASP, TENSP, GIA FROM SANPHAM SP
...
SELECT TOP 3 HOTEN, DOANHSO, RANK() OVER(ORDER BY DOANHSO DESC) AS RANK_KH FROM KHACHHANG
ORDER BY DOANHSO DESC

--32. Tính tổng số sản phẩm do “Trung Quoc” sản xuất.
SELECT COUNT(SL) AS TONGSANPHAM FROM CTHD CT
JOIN SANPHAM SP ON SP.MASP = CT.MASP
WHERE  SP.NUOCSX = 'Trung Quoc'

--33. Tính tổng số sản phẩm của từng nước sản xuất.
SELECT NUOCSX, COUNT(SL) AS TONGSANPHAM FROM CTHD CT
JOIN SANPHAM SP ON SP.MASP = CT.MASP
GROUP BY NUOCSX

--34. Với từng nước sản xuất, tìm giá bán cao nhất, thấp nhất, trung bình của các sản phẩm.
SELECT NUOCSX, MAX(GIA) GIACAONHAT, MIN(GIA) GIATHAPNHAT, AVG(GIA) GIATRUNGBUN FROM CTHD CT
JOIN SANPHAM SP ON SP.MASP = CT.MASP
GROUP BY NUOCSX
...
SELECT MASP, SUM(SL) TONGSL FROM CTHD CT
JOIN HOADON HD ON HD.SOHD = CT.SOHD
WHERE YEAR(HD.NGHD) =  2006 AND MONTH(HD.NGHD) = 10 
GROUP BY MASP

--37. Tính doanh thu bán hàng của từng tháng trong năm 2006.
SELECT MONTH(HD.NGHD) AS MONTH, SUM(TRIGIA) AS DOANHTHU FROM HOADON HD
WHERE YEAR(HD.NGHD) = 2006
GROUP BY NGHD

--38. Tìm hóa đơn có mua ít nhất 4 sản phẩm khác nhau.
SELECT SOHD, COUNT(DISTINCT MASP) SOSPKHACNHAU FROM CTHD
GROUP BY SOHD
HAVING COUNT(DISTINCT MASP) >= 4;

--39. Tìm hóa đơn có mua 3 sản phẩm do “Viet Nam” sản xuất (3 sản phẩm khác nhau).
SELECT SOHD FROM CTHD CT
JOIN SANPHAM SP ON SP.MASP = CT.MASP
...
SELECT TOP 1 KH.MAKH, HOTEN, COUNT(KH.MAKH) SLMUA FROM KHACHHANG KH
JOIN HOADON HD ON HD.MAKH = KH.MAKH
GROUP BY KH.MAKH, HOTEN
ORDER BY COUNT(HD.SOHD) DESC

--41. Tháng mấy trong năm 2006, doanh số bán hàng cao nhất ?
SELECT TOP 1 MONTH(HD.NGHD) AS MONTH FROM HOADON HD
...
SELECT TOP 1 SP.MASP, TENSP FROM SANPHAM SP
...
FROM (
	SELECT NUOCSX, MASP, TENSP, RANK() OVER(PARTITION BY NUOCSX ORDER BY GIA DESC) RANK_SP FROM SANPHAM
) T
WHERE T.RANK_SP = 1 

--44. Tìm nước sản xuất ít nhất 3 sản phẩm có giá bán khác nhau.
SELECT NUOCSX FROM SANPHAM SP
GROUP BY NUOCSX
HAVING COUNT(DISTINCT MASP) >= 3

--45. *Trong 10 khách hàng có doanh số cao nhất, tìm khách hàng có số lần mua hàng nhiều nhất.
SELECT TOP 1 T.MAKH, COUNT(HD.SOHD) SLMUAHANGMAX
FROM (
	SELECT TOP 10 MAKH, RANK() OVER(ORDER BY DOANHSO DESC) AS RANK_DS FROM KHACHHANG
) T 
JOIN HOADON HD ON HD.MAKH = T.MAKH
GROUP BY T.MAKH
ORDER BY COUNT(HD.SOHD) DESC